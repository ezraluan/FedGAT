...
class ConvNet(nn.Module):
    ...
    def _make_layers(self, channel, net_width, net_depth, net_norm, net_pooling, im_size):
        ...
        for d in range(net_depth):
            # add compatibility for image 3*28*28 in medmnist dataset
            layers += [nn.Conv2d(in_channels, net_width, kernel_size=3, padding=3 if (channel == 1 or (channel == 3 and im_size[0] == 28)) and d == 0 else 1)]
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, channel=3, num_classes=10, norm='instancenorm'):
        ...
        # self.features = nn.Sequential(*(list(self.features.children()) + list(self.layer1.children())))
        # self.features = nn.Sequential(*(list(self.features.children()) + list(self.layer2.children())))
        # self.features = nn.Sequential(*(list(self.features.children()) + list(self.layer3.children())))
        # self.features = nn.Sequential(*(list(self.features.children()) + list(self.layer4.children())))
        ## 
